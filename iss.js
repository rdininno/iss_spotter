...
const fetchCoordsByIP = (string, callback) => {
  request('https://api.ipbase.com/v2/info?apikey=7LeMME2XC2Jw8eTWj5NHdTnyRAwwUaMjTxo1T1Vp&language=en&ip=24.69.140.249',
    (error, response, body) => {
      if (error) {
        callback(error, null);
        return;
      ...
      if (response.statusCode !== 200) {
        const msg = `Status Code ${response.statusCode} when locating. Response: ${body}`;
        callback(Error(msg), null);
        return;
      }
